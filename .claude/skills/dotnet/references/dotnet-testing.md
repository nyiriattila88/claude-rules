...
General:
  ...
  Requirements:
    ...
    - Use AAA (Arrange-Act-Assert) structure in every test
...
Assertions:
  - Prefer FluentAssertions for readable assertions, and use the highest version below 8.0.0 (currently 7.0.0):
      ```xml
      <ItemGroup>
        <PackageReference Include="FluentAssertions" Version="7.0.0" />
      </ItemGroup>
      ```
  - If FluentAssertions is not used, xUnit assertions are acceptable.
  - Always use the most specific assertion for the value type and intent:
      - Numeric equality: `Be(...)` / `Assert.Equal(...)`
      - Collections: `HaveCount(...)`, `Contain(...)`, `Assert.Collection(...)`
      - Type checks: `BeOfType<T>()`, `Assert.IsType<T>(...)`
      - Exceptions: `Throw<TException>()`, `Assert.Throws<TException>(...)`
  - Avoid generic assertions when a stronger, type-specific assertion exists.
      ```csharp
      // Good: Strong, type-specific assertion
      result.Total.Should().Be(40.0m);
      result.Items.Should().HaveCount(2);

      // Avoid: Weak assertion when stronger one is possible
      result.Should().NotBeNull();
      ```
  - Example with xUnit assertions:
      ```csharp
      ...
      public class OrderTests
      {
          ...
          public void Order_WithValidData_HasExpectedProperties()
          {
              // Arrange
              var id = OrderId.New();
              OrderLine[] lines = [new OrderLine("SKU1", 1, 10.0m)];
...
Mocking and Test Data Libraries:
  - Prefer `NSubstitute` for mocking and `AutoFixture` for test data generation.
  - `Moq` is allowed only when needed for legacy compatibility, and version must be `<= 4.18.4` due to security policy.
  - Do not introduce new tests that depend on newer `Moq` versions.
      ```xml
      <!-- Preferred -->
      <ItemGroup>
      ...
      <!-- Allowed only for legacy compatibility -->
      <ItemGroup>
