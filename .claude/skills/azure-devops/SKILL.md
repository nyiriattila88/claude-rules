...
- **Ha az `azure-devops` MCP server elérhető, azt használd elsőként.** A Microsoft lokális szervere (`@azure-devops/mcp`, a repó `.claude/mcp/azure-devops.ps1` wrapperén át, a Credential Manager PAT-jával) tipizált toolokat ad repóra, PR-ra, pipeline-ra, work itemre, wikire és keresésre (`mcp__azure-devops__*`), shell-quoting és kódlap-csapdák nélkül. Az író toolok (`*_write`, `repo_create_branch`, `wiki_upsert_page`) a settingsben `ask` szabály alatt vannak, kivéve a `repo_pull_request_thread_write`-ot, ami `allow`, mert a review-kommentre állandó engedély van (lásd a PR review pontot). Az `az` CLI a fallback, és ami a szerverben nincs (retention lease, pipeline permission, `az devops invoke` REST).
...
- **PR review-nál a felhasználó legalább optional reviewer legyen.** Ha egy ADO PR review-ját kéri, és személyesen még nincs a reviewer-listán (se optional, se required, a csoport-reviewer nem számít), vedd fel `az repos pr reviewer add`-dal, ami optional reviewert csinál. Erre és a review-kommentek feltöltésére állandó engedély van, a szavazás viszont engedélyköteles marad. Részletek: reference → *PR review*.
