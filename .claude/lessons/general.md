...
- **2026-09-24, JetBrains 2026.2: a terminál shellje a `terminal-local.xml`-ben van, a Markdown-futtató a projekt-gyökérben fut:** a `terminal.xml` `myShellPath`-je deprecated, a `TerminalLocalOptions` `shellPath`-je él (kívülről létrehozva, újraindítás után működött). A `README` ▶ parancsai a projekt base directoryjában futnak, a fájl mappájában csak a `markdown.command.runner.use.file.directory` registry-kulccsal. A registry az `ide.general.xml`-ben van, amit a futó IDE felülír, azt a felületen állítsd.
...
- **2026-09-25, egy branch history-átírása visszaveszi a GitVersion verzióját:** a pre-release szám a commitok számából jön, így 266 commit 30-ba sűrítése után a build `0.1.0-alpha.266`-ról `alpha.30`-ra esett. Force push előtt mondd ki, és ha a verziónak monoton kell nőnie, előbb tag vagy `next-version` kell.
...
- **2026-09-25, az `az pipelines runs list --top N` kihagyja a futó run-okat:** két Deploy "nem indult el", mert a `--top 8` és a `--pipeline-ids` lista csak befejezett run-okat mutatott, pedig mindkettő percek óta futott. A REST builds lista `statusFilter=all` és `queryOrder=queueTimeDescending` mellett megmutatta őket, előbb ezt kérdezd.
