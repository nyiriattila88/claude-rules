...
- **Ha az `ado` MCP server elérhető, azt használd elsőként.** A Microsoft lokális szervere (`@azure-devops/mcp`, a repó `.claude/mcp/ado-mcp.ps1` wrapperén át, a Credential Manager PAT-jával) tipizált toolokat ad repóra, PR-ra, pipeline-ra, work itemre, wikire és keresésre (`mcp__ado__*`), shell-quoting és kódlap-csapdák nélkül. Az író toolok (`*_write`, `repo_create_branch`, `wiki_upsert_page`) a settingsben `ask` szabály alatt vannak. Az `az` CLI a fallback, és ami a szerverben nincs (retention lease, pipeline permission, `az devops invoke` REST).
