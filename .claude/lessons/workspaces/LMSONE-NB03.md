...
- **2026-09-23, MCP szerverek ezen a gépen:** user-scope `ado` (a repó `.claude/mcp/ado-mcp.ps1` wrappere, PAT a Credential Managerből) és `aws-mcp` (SigV4, frankfurti végpont, `dev qa stg prod sandbox` profilok, mind `READONLY-*` role). Indulás: ADO 25 s meleg, 56 s hideg, AWS kb. 50 s öt profillal. Lejárt SSO-nál az `aws-mcp` el sem indul, `aws sso login --sso-session aether-sso` kell.
