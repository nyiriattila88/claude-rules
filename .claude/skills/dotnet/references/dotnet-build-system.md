...
globs: "**/Directory.Build.props,**/.azure/*.yaml,**/build-system/*.yaml,**/RELEASE_NOTES.md,**/.github/workflows/*.yaml,**/*.sln,**/*.slnx,**/*.csproj"
alwaysApply: false
---
...
#### ✅ DO: Run restore as a dedicated locked step
```yaml
steps:
  - script: dotnet restore --locked-mode -c Release
    displayName: 'Restore (locked mode, Release, warnings as errors)'
...
#### ❌ DON'T: Mix restore with build/test
```yaml
steps:
...
jobs:
  - job: ${{ parameters.name }}
    ...
    steps:
      ...
      - script: dotnet restore --locked-mode -c Release
        displayName: 'Restore (locked mode, Release)'
...
- Skip test runs
- Ignore build or restore warnings (enforce via Release + `TreatWarningsAsErrors` and restore with `-c Release`; see `dotnet-locked-restore-ci.md`)
...
#### ✅ DO:
- Use `dotnet restore --locked-mode -c Release` in CI/CD (matches Release warning-as-error policy on restore)
- Let `dotnet` CLI handle all build operations
- Use `--no-restore` after successful restore
