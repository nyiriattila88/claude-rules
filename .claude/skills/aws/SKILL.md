...
- **A hivatalos skill is kötelező, és hook kényszeríti ki.** Ugyanabban a turnben, amelyikben ez a skill betöltődik, és még az első AWS-hívás előtt töltsd be a feladathoz illő `aws-core:` skillt is. Ahol a hookok be vannak kötve, a `pre-tool-use-official-skills.ps1` minden `aws` CLI- és `aws-mcp`-hívást elutasít, amíg a transcript nem mutatja mindkettő betöltését. Ilyenkor töltsd be őket, és futtasd újra ugyanazt a hívást, ne kerüld meg (pl. boto3-as scripttel).
- **Account-hívásra az aláíró `aws-mcp` szerver kell, nem a pluginé.** A plugin saját szervere (`mcp__plugin_aws-core_aws-mcp__*`) `--skip-auth`-tal indul, és default profil híján aláíratlanul küld, tehát csak dokumentációra jó. Ahol be van kötve a user-scope `aws-mcp` (`mcp__aws-mcp__*`, `mcp-proxy-for-aws` SigV4-gyel, read-only role-os profilokkal), ott az account-hívás az `aws___run_script`: Python egy AWS-oldali sandboxban, ahol a `boto3` import **tiltott**. Az API-t az előre betöltött `await call_boto3(service_name="sts", operation_name="GetCallerIdentity", params={...})` hívja, az eredmény egy `result` változó, utolsó kifejezésként. A profilt az `aws_profile` paraméter választja. Lejárt SSO-nál a szerver el sem indul (a proxy `TokenRetrievalError`-ral áll meg, a `claude mcp list` `-32602`-t mutat, a `--skip-auth` ezen nem segít), ilyenkor `aws sso login` kell a felhasználótól és új session. A működő `aws sts` ezt nem cáfolja.
...
- **Natív megoldás a sajátod előtt.** Mielőtt levezetsz egy értéket, összeraksz egy működést komponensekből, vagy kimondod, hogy „erre nincs beépített út", nézd meg: publikálja-e az AWS metrikaként, adja-e feature-ként. Ez a képesség szokott újabb lenni, mint a betanított tudás, ezért pont ez az a hit, amit ellenőrizni kell. Egy levezetett érték elcsúszik attól, amit közelít, és ellentmondhat annak az AWS-komponensnek, ami az igazi metrikát olvassa. Részletek és két konkrét eset: `references/aws-documentation.md`.
