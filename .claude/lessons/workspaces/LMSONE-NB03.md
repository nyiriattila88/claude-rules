...
- **2026-09-24, a `py` nem látja a session scratch workspace-ét:** az `AppData\Roaming\Claude\scratch-workspaces\...` alá írt scriptre `No such file or directory`-t ad, miközben a Git Bash `ls`-e és a `git` látja (valószínűleg a Desktop app MSIX-virtualizált Roaming mappája). Python-scriptet és -adatot a scratchpadbe (`AppData\Local\Temp\claude\...`) tegyél.
- **2026-09-24, a scratchpadbe nem lehet klónozni:** az útvonala kb. 230 karakter, a `git clone` a `.git/config` olvasásánál `Filename too long`-gal bukik, és a `-c core.longpaths=true` sem segít. Repót a session scratch workspace-ébe klónozz (kb. 157 karakter).
