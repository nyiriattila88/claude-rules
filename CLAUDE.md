...
| `azure-devops` | bármely Azure DevOps munka: `az devops`/`repos`/`pipelines`/`boards`, PR, pipeline run, build timeline, work item, PAT/setup | Azure DevOps CLI: PAT-alapú auth (nem `az login`), telepítés-ellenőrzés telepítés előtt, PowerShell `--query` quoting trap, read-only vs. engedélyköteles mutáló parancsok, `az devops invoke` REST fallback, token-takarékos lekérdezés |
