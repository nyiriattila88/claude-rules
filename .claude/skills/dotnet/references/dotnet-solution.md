...
globs: "**/*.sln,**/*.slnx,global.json,**/Directory.Build.props,**/Directory.Packages.props,**/*.csproj,**/*.fsproj"
alwaysApply: false
---

# Claude Rules File: Best Practices for .NET Solution Management
...
SDK Version Management:
  - Maintain a global.json file in the solution root:
      ...
      - Explicitly set allowPrerelease to false for stable SDK resolution
      - Example:
        ```json
        {
          "sdk": {
...
Shared Build Properties:
  ...
  - Disable implicit usings; use explicit global usings via GlobalUsings.cs:
      - Do not enable SDK implicit usings (`<ImplicitUsings>enable</ImplicitUsings>`). Keep full control over which namespaces are in scope.
      - Use a dedicated `GlobalUsings.cs` (or `GlobalUsings/*.cs`) in the project with explicit `global using` directives for project-wide namespaces.
      - Set in Directory.Build.props or per-project:
        ```xml
        <PropertyGroup>
          <ImplicitUsings>disable</ImplicitUsings>
        </PropertyGroup>
        ```
      - DO: One or more GlobalUsings.cs files with explicit `global using System;`, `global using System.Collections.Generic;`, etc., so dependencies are visible and reviewable.
      - DON'T: Rely on implicit usings; they hide which namespaces are available and can differ by SDK/project type.
...
Compilation:
  ...
  - Enforce code quality:
      - Enable `TreatWarningsAsErrors` **for Release only** in `Directory.Build.props` using a **Configuration** condition (CI and publish use `-c Release`; Debug stays more permissive for local work). Run **`dotnet restore -c Release`** in CI (e.g. `dotnet restore --locked-mode -c Release`) so **restore does not emit ignored NU\* warnings** that would not fail under Debug.
        ```xml
        <PropertyGroup Condition="'$(Configuration)' == 'Release'">
          <TreatWarningsAsErrors>true</TreatWarningsAsErrors>
          <!-- Optionally allow specific warnings in Release -->
