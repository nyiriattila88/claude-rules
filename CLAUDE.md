...
After linking, `dotnet`, `typescript`, `terraform`, `aws`, `powershell`, `github`, `azure-devops`, `local-code-review`, and `devils-advocate-review` are triggerable from any project.
...
| `azure-devops` | bármely Azure DevOps munka: `az devops`/`repos`/`pipelines`/`boards`, PR, pipeline run, build timeline, work item, scheduled (cron) trigger, PAT/setup, „nem érem el az ADO-t" | Azure DevOps CLI: **elérés, explicit `--org` + `--detect false`** (a „you need to run the login command" hiba az org-detect, nem a PAT; a beállított default nem védi meg), PAT-alapú auth (nem `az login`), telepítés-ellenőrzés telepítés előtt, PowerShell/`az.cmd` csapdák (`--query` zárójel, markdown `\|` és backtick, `ConvertFrom-Json` tömb), read-only vs. engedélyköteles mutáló parancsok, `az devops invoke` REST fallback, token-takarékos lekérdezés, koncepció-validálás temporary change-dzsel (jelöl → validál → kötelező revert), trigger-szabályok (cron az adott ág YAML-jéből, pipeline-completion a pipeline default branchéből), félrevezető futás-metaadata (`reason`, indexelési késés), pipeline resource artifact-választás, PR review-nál a felhasználó legalább optional reviewer (állandó engedély) |
| `powershell` | PowerShellből natív CLI hívása (`az`, `aws`, `gh`, `git`, `tofu`, `dotnet`), `.ps1` írása vagy futtatása, fájlírás PowerShellből, JSON átadása egy CLI-nek, ékezetes szöveg keresztülvitele egy eszközön | A natív exe határ: a PowerShell **kiszedi a belső dupla idézőjeleket** (JSON és `for_each` cím csak fájlon vagy escape-elve adható át), a `--query`/`--jq` elhasal a wrapperen. Encoding: a `ConvertTo-Json` **nem** escape-el, így az ASCII-írás minden ékezetet kérdőjelre cserél, és a „nulla non-ASCII bájt" ellenőrzés hamis biztonságot ad; a visszaolvasás sosem bizonyíték. Fájlírás `WriteAllBytes`-szal (`Set-Content` ANSI-t, `Out-File` BOM-ot ír), de az ékezetes `.ps1` BOM-ot **igényel**. Hibakezelés: egy hibára futó mutáló hívás mellett a művelet lefuthatott |
| `github` | `gh` CLI: PR, issue, `gh run` (Actions), release, `gh api`, repo-lista, „nem tudok pusholni", push 403 | A push **403**-ja a rossz **aktív fiók**, nem a token-scope (`gh auth switch`, ne új PAT); az üres `gh api user/orgs` sem bizonyítja a hozzáférés hiányát. A `could not read Password ... terminal prompts disabled` a `!` prefixes credential helper, ami shellt indít (egyszeri `-c credential.https://github.com.helper=manager` megkerülés), nem lejárt token. A `--jq` PowerShell alól szétesik. Read szabad, a `gh workflow run` / `run rerun` / `release create` / `repo delete` engedélyköteles; többsoros szöveg `--body-file`-lal |
