...
- **PR-kommentet Git Bashből küldj, ne PowerShellből.** A thread API resource-neve `pullRequestThreads` (a `threads` névre kapott hiba félrevezetően az `--api-version`-re mutat), a szerkesztés `pullRequestThreadComments`. PowerShell alól az `az.cmd` wrapperen a **nem-ASCII karakterek csendben elvesznek** a bodyból (a hívás `200`-at ad), `\uXXXX`-escape és `PYTHONUTF8=1` sem segít — Bashből ugyanaz a body hibátlan. Magyar, ékezetes kommentnél ez adatvesztés: a sikert a válaszban visszakapott `content`-en ellenőrizd. Lásd [[git-conventions]] a nyelvről és az AI-marker tilalmáról.
